...
namespace HeThongQuanLyCongTacKhaoThi.ApiIntegration
{
    public class AccountApiClient : IAccountApiClient
    {
        ...
        public AccountApiClient(IHttpClientFactory httpClientFactory, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<ApiResult<AccountViewModel>> GetByID(Guid id)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<AccountViewModel>>(body);
            }

            return JsonConvert.DeserializeObject<ApiErrorResult<AccountViewModel>>(body);
        }
        ...
        public async Task<ApiResult<bool>> UpdateAccount(Guid id, AccountUpdateRequest request)
        {
            var sessions = _httpContextAccessor.HttpContext.Session.GetString("Token");
            var client = _httpClientFactory.CreateClient();
            var json = JsonConvert.SerializeObject(request);
        ...
        public async Task<ApiResult<bool>> RoleAssign(Guid id, RoleAssignRequest request)
        {
            ...
            client.BaseAddress = new Uri(_configuration["BaseAddress"]);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", sessions);
            var response = await client.PutAsync($"/api/accounts/{id}/roles", httpContent);
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<bool>>(result);
        ...
        public async Task<ApiResult<AccountViewModel>> GetByUserName(string username)
        {
            ...
            client.BaseAddress = new Uri(_configuration["BaseAddress"]);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", sessions);
            var response = await client.GetAsync($"/api/accounts/username/{username}");
            var body = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<AccountViewModel>>(body);
        ...
        public async Task<ApiResult<List<AccountViewModel>>> GetAllTeachers()
        {
            ...
            client.BaseAddress = new Uri(_configuration["BaseAddress"]);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", sessions);
            var response = await client.GetAsync($"/api/accounts/get-all-teachers");
            var body = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<List<AccountViewModel>>>(body);
            }

            return JsonConvert.DeserializeObject<ApiErrorResult<List<AccountViewModel>>>(body);
        }
    }
}